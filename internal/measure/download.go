...
const (
	...
	minTestDuration = 10 * time.Second
)
...
func Download(targets []string, duration time.Duration, progressCh chan<- Progress, verboseCh chan<- VerboseLog, headers map[string]string) error {
	if len(targets) == 0 {
	...
	for _, target := range targets {
		...
		go func(url string) {
			...
			for {
				...
				if err := downloadChunk(url, &totalBytes, done, log, headers); err != nil {
					return
				}
			}
		}(target)
...
func downloadChunk(url string, totalBytes *atomic.Int64, done <-chan struct{}, log func(string), headers map[string]string) error {
